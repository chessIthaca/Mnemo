...
Amended 2027-01-11: The git tool's schema now carries `required: ["subcommand"]` and its description ends with a per-subcommand required-fields map (commit→message, merge/checkout→branch, restore→paths, branch delete/create→branch). Every per-subcommand required-field validation error now quotes the exact expected arguments — e.g. the commit error carries `{"subcommand": "commit", "message": "<commit message>"}` plus the sibling-field rejection note (branch is merge/checkout-only, paths/source/target are restore-only) — and the missing-subcommand error carries the same map. The earlier "no `required` array, by design" note is superseded for `subcommand` only (uniformly true; the forgiving action-carrier resolution still accepts action-only calls at runtime). Landed with the repeat-failure circuit breaker (plan a9875b73): a second identical tool failure now also triggers a harness-injected schema correction, so these enriched errors are the first line and the breaker the backstop.

Amended 2027-01-24: the `branch list` read-only allowlist now also accepts `--show-current` (exact flag — prints the checked-out branch's name, exactly as read-only as --list) and `--points-at=<object>` (prefix form — a read-only listing filter; the `=` form keeps the value off the positional path). Audited and staying excluded: `--column`/`--no-column`/`--omit-empty` (display-only formatting) and `-l` (deprecated synonym of `--list`); every mutating flag stays refused. The rejection message's flag enumeration is kept in sync with the allowlist.
