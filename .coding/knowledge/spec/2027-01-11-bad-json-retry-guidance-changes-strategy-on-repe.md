...
Amended 2027-01-11: the advertised surface is now complete and PINNED — both tools' `pattern` param descriptions carry "Prefer literal:true for text with regex metacharacters or backslashes." (the param a model reads when formulating the pattern), and the tests schema_advertises_the_literal_escape_hatch (search.rs + search_read.rs, mirroring the read_files collapse test from plan 9e0b266a) assert the ESCAPE-HATCH marker, the recovery rule, the example (search), the RECOMMENDED `literal` wording, and the pattern-param recommendation — the text can never silently disappear. Budget: all six filters under ceiling (Planning/Complete 18,869/18,900, 31 headroom).
