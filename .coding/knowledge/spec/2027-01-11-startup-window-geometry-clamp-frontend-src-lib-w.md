...
Amended 2027-01-11: Landed outcome (2027-01-14): shipped in commit c9da825 on branch wt/mnemo (merge_to_main pending — branch is unmerged). Review round 1: FINDINGS (0 high / 8 low, all fixed — see .coding/reviews/2026-09-14-window-geometry-restore-clamp-review.md); review round 2: PASS (.coding/reviews/2026-09-14-window-geometry-restore-clamp-review-round2.md, all 23 test expectations hand-re-derived). Verification: vitest 1135 passed / 83 files, `npm run build` clean, `cargo test` 2359 + 16 passed / 0 failed. Root-cause SPEC companion: "min_inner_size bounds user resize only — programmatic setSize is unclamped (tao WM_GETMINMAXINFO)". The auto-captured BUG digest (da74019a-e513-5184-b6a5-9235b62c5366) re-derives from the plan file, which carries the full landed-design amendment.
