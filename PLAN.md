...
L4): while a plan is active (Executing/Reviewing), the loop builds the `tools`
array from `Workflow::schema_filter()` — `PlanFrozen` (the Executing surface ∪
`finish`) for implementation/bug-fixing plans, `ExecutingResearch` for research
plans — chosen once at plan activation by plan kind and byte-stable for the
plan's whole lifetime. The provider prefix cache reuses the request body only
...
are still rejected at dispatch — advertisement is frozen, enforcement is not.
