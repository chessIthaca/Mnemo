...
impl InnerWebView {
  ...
  // Mnemo patch (see PATCHES.md): a HARD reload — WebView2's Reload()
  // serves cached subresources, so the Browser tab went stale; CDP
  // Page.reload with ignoreCache is the Ctrl+Shift+R equivalent. Falls
  // back to Reload() when the CDP call itself fails.
  pub fn reload(&self) -> Result<()> {
    ...
    let called = unsafe {
      self.webview.CallDevToolsProtocolMethod(
        &method,
        &params,
        // fire-and-forget: the completion only echoes the call's JSON
        // result — nothing to wait for
        &CallDevToolsProtocolMethodCompletedHandler::create(Box::new(
          move |_error_code, _result_object_as_json| Ok(()),
        )),
      )
    };
    match called {
      Ok(()) => Ok(()),
      Err(_) => unsafe { self.webview.Reload() }.map_err(Into::into),
    }
  }
